...
class TimetableGenerator:
    ...
    def generate_timetables(self):
        if not os.path.exists('timetables'):
            ...
            for layer in self.mlgraph.layers.keys():
                if 'CAR' not in layer.upper():
                    print('CHECK LAYER', layer)
                    print(self.mlgraph.layers)
...
class CongestionModel:
    ...
    def __init__(self, mlgraph=None):
        """
        ...
        if CongestionModel.__instance is not None:
            ...
            self.in_vehicle_data = pd.DataFrame(columns=['TIMESTAMP', 'VEHICLE ID', 'PASSENGERS', 'CAPACITY', 'CONGESTION INDEX', 'NODE'])
            self.waiting_data = pd.DataFrame(columns=['TIMESTAMP', 'NODE', 'EXPECTED'])
            self.mlgraph = mlgraph
            self.capacity_info = {'BUSA': 30, 'BUSB': 30, 'METRO': 150, 'TRAM': 70}
            #self.timetables = TimetableGenerator(self.mlgraph).get_line_timetables()


    @staticmethod
    def get_instance(mlgraph=None):
        """
        Static method to get the singleton instance.
        Creates the instance if it doesn't exist.
        """
        if CongestionModel.__instance is None:
            CongestionModel.__instance = CongestionModel(mlgraph)
    ...
    def update_congestion_model_mavg(self, new_data, model='moving_avg'):
        if model == 'moving_avg':
            ...
            self.in_vehicle_data = pd.concat([self.in_vehicle_data, next_row], ignore_index=True)  # Concatenate with existing DataFrame
    ...
    def update_or_add(self, t, node, line):
        ...
        key = (timestamp, node)
        existing_keys = list(zip(self.waiting_data['TIMESTAMP'], self.waiting_data['NODE']))
        if key in existing_keys:
            # Key exists, increment 'EXPECTED'
            index = existing_keys.index(key)
            value = self.waiting_data.loc[index, 'EXPECTED'] + 1
            self.waiting_data.loc[index, 'EXPECTED'] = value
        else:
            # Key doesn't exist, append the row
            new_row = {'TIMESTAMP': timestamp, 'NODE': node, 'EXPECTED': 1}
            self.waiting_data = pd.concat([self.waiting_data, pd.DataFrame([new_row])], ignore_index=True)
        self.waiting_data.to_csv('expected_congestion.csv', index=False)
    ...
    def predict_congestion_mavg(self, t, node, line):
        CI = self.in_vehicle_data[self.in_vehicle_data['NODE'] == node]
        ...
        val = CI_mod['CONGESTION INDEX MA'].iloc[-1]
        print(f'Computing congestion {node} {val}')
        return val

    def predict_waiting(self, t, node, line):
        t = self.align_to_next_departure(t, node, line)
        print('PREDICT CONGESTION for ', t, node, line)
        CI = self.waiting_data[self.waiting_data['NODE'] == node].copy(deep=True)
        print(CI.iloc[:, :])
        if len(CI) == 0:
            print('NO CONGESTION INFO for node', node)
            return 0
        else:
            CI = self.waiting_data[self.waiting_data['TIMESTAMP'] >= t].reset_index(drop=True)
            if len(CI) == 0:
                print('NO CONGESTION INFO for time', t, node)
                return 0
            else:
                print('CONGESTION INFO for time', t, node, CI.loc[0, 'EXPECTED'])
                return CI.loc[0, 'EXPECTED'] / self.capacity_info[line]

    def clear_data(self):
        """
        Clears all congestion data.
        """
        self.in_vehicle_data.clear()
        self.waiting_data.clear()

    def write_congestion(self, path):
        self.in_vehicle_data.to_csv(f'{path}{os.sep}congestion_history.csv')
        self.waiting_data.to_csv(f'{path}{os.sep}expected_waiting_history.csv')
