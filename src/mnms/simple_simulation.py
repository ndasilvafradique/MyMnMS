...
class Supervisor():
    def __init__(self,
        ...
        self.cm = CongestionModel.get_instance()
    ...
    def run(self, tstart: Time, tend: Time, flow_dt: Dt, affectation_factor: int, update_graph_threshold: float = 0., seed: int=None):
        ...
        #self.cm = CongestionModel.get_instance()
        CongestionModel.get_instance(self._mlgraph)
            ...
            for _ in range(affectation_factor):
                ...
                for vehicle_id in VehicleManager._vehicles:
                    if VehicleManager._vehicles[vehicle_id].is_moving:
                        ...
                        # if VehicleManager._vehicles[vehicle_id].activity_type != ActivityType.REPOSITIONING:
                            # and  VehicleManager._vehicles[vehicle_id].activity_type != ActivityType.STOP):
        ...
        ### Finalize simulation
        f.close()
        #self.cm.write_congestion('OUTPUTS')
    ...
    #     # Ensure nested objects are also serialized correctly
    #     if "decision_model" in state and hasattr(self.decision_model, "__getstate__"):
    #         state["decision_model"] = self.decision_model.__getstate__()
    #     return state

    # def save_state(self, filename="simulation.fs"):
    #     """Save the simulation state as a checkpoint."""
    #     # with open("simulation.bson", "wb") as f:
    #     #     f.write(BSON.encode(sim.to_dict()))
    #     storage = FileStorage.FileStorage(filename)
    #     db = DB(storage)
    #     conn = db.open()
    #     root = conn.root()
    #
    #     root.simulation = self  # Store the simulation object
    #     transaction.commit()  # Save changes
    #
    #     db.close()
    #     print(f"✅ Simulation state saved to {filename}")
    #
    #     # """Saves the entire simulation state to a file"""
    #     # state_to_save = dict()
    #     # for k,v in self.__dict__.items():
    #     #     if k not in ['_write', '_outfile', '_csvhandler', '_demand',
    #     #                  #'_user_flow',
    #     #                  '_decision_model',
    #     #                  '_flow_motor'
    #     #                  ]:
    ...
    #     #             state_to_save[k] = v.__getstate__()
    #     #
    #     #         if type(v) is not MultiLayerGraph:
    #     #             state_to_save[k] = v
    #     #         else:
    #     #             save_graph(v, f"{self.tcurrent.to_seconds()}_mmgraph.json")
    #     # filename = f"checkpoint_{self.tcurrent.to_seconds()}.json"
    #     # joblib.dump(state_to_save, filename)
    #     # print(f"Simulation state saved to {filename}")
    #
    # def load_state(filename="simulation.fs"):
    ...
    #
    #     root = conn.root()
    #     if hasattr(root, "simulation"):
    #         simulation = root.simulation  # Retrieve the stored object
    #         print(f"✅ Loaded simulation: {simulation.name}")
    #         return simulation
    #     else:
    #         print("⚠️ No saved simulation found.")
    #         return None
